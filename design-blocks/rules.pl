% Rule to add a course fact
add_course(CourseCode, CourseName, Credits, Semester, DifficultyScore, Prerequisites) :-
    assert(course(CourseCode, CourseName, Credits, Semester, DifficultyScore, Prerequisites)).

% Rule to delete a course
delete_course(CourseCode) :-
    find_course(CourseCode, _, _, _, _, _), % Find the course
    retract(course(CourseCode, _, _, _, _, _)). % Retract the found course
...
% Rule to find the difficulty score of a course
...
generate_year_plan(Year, [Semester | Rest]) :-
    Year < 4,
    generate_semester([], 2.3, Semester), % Adjust the difficulty score as needed
    NextYear is Year + 1,
